...
### Added
- Game hubs show **match cards**: each play session is one card in the grid — its best moment's event badge, a clip count, and (for League) a live **kills / deaths** summary. Tapping a card opens that match's clips. Deaths are now tracked from the Live Client API (you as the victim) and, with kills, persisted per match in `matches.json`.
