...
- **Real game icons in the rail** — read from each game's installed app bundle at runtime, or, for Steam games with no macOS bundle (CrossOver/Wine titles like R.E.P.O.), from the local Steam library's own cached art; no bundled artwork (League keeps its monogram: its app icon is Riot's official logo, which Riot's asset policy excludes even though champion/item art is explicitly permitted)
